...
class HrEmployeeContract(models.Model):
    ...
    shift_schedule = fields.One2many('employee.hr.shift.schedule', 'rel_hr_schedule', string="Shift Schedule", help="Shift schedule")
...
class HrSchedule(models.Model):
    _name = 'employee.hr.shift.schedule'
    ...
    def _check_overlap(self, vals):
        if vals.get('start_date', False) and vals.get('end_date', False):
            shifts = self.env['employee.hr.shift.schedule'].search([('rel_hr_schedule', '=', vals.get('rel_hr_schedule'))])
            for each in shifts:
                if each != shifts[-1]:
