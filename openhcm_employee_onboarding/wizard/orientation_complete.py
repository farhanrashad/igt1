...
class OrientationForceComplete(models.TransientModel):
    _name = 'employee.orientation.force.complete'

    name = fields.Char()
    orientation_id = fields.Many2one('employee.orientation', string='Orientation')
    orientation_lines = fields.One2many('employee.orientation.request', string='Orientation Lines', compute='pending_lines')
